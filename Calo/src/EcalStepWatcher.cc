...
EcalStepWatcher::EcalStepWatcher(const edm::ParameterSet& iConfig)
{
	...
	image = iConfig.getParameter<bool>("image_only");
	  ...
	  //tree_->Branch("image", "vector<double>", &entry_.image, 32000, 0);
	  tree_->Branch("histogram", "TH2F", &entry_.h2, 32000, 0);
...
void EcalStepWatcher::update(const BeginOfEvent* evt) {  
        //reset branches
  	entry_ = SimNtuple();
  	entry_.h2 = fs_->make<TH2F>("h", "hist", xbins, xmin, xmax, ybins, ymin, ymax);
...
/*	Int_t x, y;
	for (x=0; x < xbins; x++){
	  for (y=0; y < ybins; y++){
	    entry_.image.push_back(entry_.h2->GetBinContent(x, y));
	  }
	}
*/
	//fill tree
